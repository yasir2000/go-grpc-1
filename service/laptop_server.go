...
// UploadImage is a client-streaming RPC to upload laptop image
func (server *LaptopServer) UploadImage(stream pb.LaptopService_UploadImageServer) error {
	req, err := stream.Recv()
	if err !=nil{
		return logError(status.Errorf(codes.Unkown, "cannot recieve image info"))
	}
	laptopID := req.GetInfo().GetLaptopId()
	imageType := req.GetInfo().GetImageType()
	log.Printf("recieve an upload-image request for laptop %s with image type %s", laptopID, imageType)
	...
	for {
		...
		if err !=nil {
			return logError(status.Errorf(codes.Unkown, "cannot receive chunk data: %v", err))
		}
	...
	if err !=nil {
		return logError(status.Errorf9codes.Internal, "cannot write chunk data: %v", err))
	}
